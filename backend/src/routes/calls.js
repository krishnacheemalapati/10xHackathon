...
function createCallRoutes(aiService, notificationService) {
    const router = (0, express_1.Router)();
    // In-memory store for demo (replace with database in production)
    const calls = new Map();
    // POST /api/calls/schedule - Schedule a new wellness check call
    router.post('/schedule', (req, res) => __awaiter(this, void 0, void 0, function* () {
        try {
            const { userId, scheduledTime, phoneNumber } = req.body;
            ...
            if (phoneNumber) {
                ...
                yield notificationService.sendSMS(phoneNumber, message);
            }
            const response = {
                callId,
                scheduledTime: new Date(scheduledTime),
    ...
    router.post('/:callId/start', (req, res) => __awaiter(this, void 0, void 0, function* () {
        try {
            const { callId } = req.params;
            const call = calls.get(callId);
            if (!call) {
